...
namespace ecommerce.API
{
    ...
    public class CarrinhoAPIController : ControllerBase
    {
        ...
        public CarrinhoAPIController(AppDbContext context)
        {
            _context = context;
        }
            ...
            [FromForm] int? idProduto = 0, [FromForm] int? quantidade = 0)
        {
            ...
            var pedido = await _context.Pedido.Include("ItensPedido").
                FirstOrDefaultAsync(p => p.IdCarrinho == idCarrinho);

            if (pedido != null)
            {
                if (pedido.Situacao == Models.PedidoModel.SituacaoPedido.Carrinho)
                {
